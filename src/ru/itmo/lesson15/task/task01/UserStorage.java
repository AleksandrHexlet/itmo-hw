package ru.itmo.lesson15.task.task01;
...
public class UserStorage {
    ...
    public void addUser(User user){ // добавление пользователя в map
        ...
        Objects.requireNonNull(user, "Exeption: user == null");
    ...
    public Map<Integer, List<User>> getUsersByAge(){
        ...
        if (users == null) throw  new IllegalArgumentException("пустая ссылка. Ссылка равна null");
        if (users.size() == 0 ) throw  new IllegalArgumentException("no have user");
    ...
    public EnumMap<User.Role, Integer> getNumberOfUsersByRole(){
        ...
        EnumMap<User.Role,Integer> enumMap = new EnumMap<User.Role, Integer>(User.Role.class);
        for (User user: users.values()){
