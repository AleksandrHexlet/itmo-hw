...
public class ServerApp {
    ...
    public void run(){
        try (ServerSocket serverSocket = new ServerSocket(port)){
            ...
            while (true) {
                ...
                try {
                    System.out.println("read(readWrite) === " + read(readWrite).getText().toString() );
    ...
    private void sendResponse(ReadWrite readWrite, Message requestMessage) { // выполнение запрошенной задачи
       ...
       System.out.println("sendResponse. requestMessage.getText() == " + requestMessage.getText());
    ...
    private void write(ReadWrite readWrite, Message message) {
        try {
            System.out.println("ServerApp.write send message to All client from method write : " + message.getText());
    ...
    private Message read(ReadWrite readWrite){
        ...
        try {
            System.out.println("read message == " + readWrite.readMessage().getText());
            fromClient = readWrite.readMessage(); // чтение сообщения от клиента
            System.out.println("Client send to server. Server.read === " +  fromClient.getText() );
        } catch (IOException e) {
